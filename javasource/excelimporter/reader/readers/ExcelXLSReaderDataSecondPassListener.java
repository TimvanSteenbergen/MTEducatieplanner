...
public class ExcelXLSReaderDataSecondPassListener implements HSSFListener {
		...
		try {
			...
			case FormulaRecord.sid:
				if (this.sheetNow == this.iCanHasSheet) {
					...
					// TODO: Use frec.getCachedResultType (?) to get other result types (e.g.
					// boolean); is resulting int
					// a sid?
			...
			case SSTRecord.sid:
				...
				if (ExcelReader.logNode.isDebugEnabled())
					ExcelReader.logNode.debug("XLS_DATA-2ndPass: Static String Table found .. filling sstlist with "
							+ this.sstmap.size() + " Strings");
			...
			case EOFRecord.sid:
				if (this.sheetNow == this.iCanHasSheet) {
					if (this.values.length > 0) {
						...
						this.rowCounter++; // This is for the last record(row)
					}
	...
	public String formatNumberDateCell(CellValueRecordInterface cell) {
		...
		if (formatString != null) {
			...
			if (ExcelReader.logNode.isTraceEnabled())
				ExcelReader.logNode
						.trace("Formatting " + (cell.getRow() + 1) + ExcelReader.colNumberToText(cell.getColumn())
								+ " / '" + value + "' using format: '" + formatString + "' as " + returnValue);
	...
	public int getFormatIndex(CellValueRecordInterface cell) {
		...
		if (xfr == null) {
			ExcelReader.logNode.error("Cell " + cell.getRow() + "," + cell.getColumn() + " uses XF with index "
	...
	private void processRecord(int row, short col, Object rawValue, Object formattedValue, String displayMask) {
		if (this.mayUseValue(row, col)) {
			if (this.rowNow != row) {
				if (row > this.startRow) {
					try {
						...
						throw new ExcelRuntimeException("XLS_DATA-2ndPass: an exception occurred near row: " + row
			...
			try {
				if (ExcelReader.logNode.isTraceEnabled())
					ExcelReader.logNode
							.trace("Reading " + (row + 1) + ExcelReader.colNumberToText(col) + " / '" + rawValue + "'");

				this.values[col] = new ExcelCellData(col, rawValue, formattedValue, displayMask);
			} catch (Exception e) {
				throw new ExcelRuntimeException("XLS_DATA-2ndPass: FormulaRecord row: " + (row) + " col: " + col
						+ ", Value: " + rawValue + "|" + formattedValue + " triggers Exception: " + e.getMessage(), e);
			}
		}
	}
